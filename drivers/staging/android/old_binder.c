...
#include "old_binder_thread.h"
#include "old_binder_node.h"
#include "old_binder_transaction.h"
#include "old_iobuffer.h"
...
struct kmem_cache *thread_cache = NULL;
struct kmem_cache *node_cache = NULL;
struct kmem_cache *local_mapping_cache = NULL;
...
static int __init init_binder(void)
{
	...
	void* mem = kzalloc(sizeof(*simple), GFP_KERNEL);
	
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,15)
	simple = CLASS_SIMPLE_DEVICE_ADD(binder_class, dev, NULL, "%s", BINDER_NAME);
#else
	// ARGH API CHANGE!!!
	simple = CLASS_SIMPLE_DEVICE_ADD(binder_class, NULL, dev, NULL, "%s", BINDER_NAME);
#endif
	if (IS_ERR(simple)) {
...
void *dbg_kmem_cache_alloc(struct kmem_cache *a, unsigned int b)
{
	dbg_mem_header_t *p;
...
void dbg_kmem_cache_free(struct kmem_cache *a, void *b)
{
