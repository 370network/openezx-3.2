...
struct binder_stats {
	int br[brFAILED_REPLY + 1];
	int bc[bcDEAD_BINDER_DONE + 1];
...
struct binder_thread {
	...
	struct list_head todo;
	uint32_t return_error; /* write failed, return error code in read buffer */
	uint32_t return_error2; /* write failed, return error code in read buffer used when sending a reply to a dead process that we are also waiting on */
	wait_queue_head_t wait;
	struct binder_stats stats;
};
 ...
 */
static void task_fd_install(struct task_struct *tsk, unsigned int fd, struct file * file)
{
	struct files_struct *files = get_files_struct(tsk);
	struct fdtable *fdt;
...
static void binder_insert_free_buffer(struct binder_proc *proc, struct binder_buffer *new_buffer)
{
	...
	if(binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_INFO "binder: %d: add free buffer, size %d, at %p\n", proc->pid, new_buffer_size, new_buffer);
...
static struct binder_buffer *binder_alloc_buf(struct binder_proc *proc, size_t data_size, size_t offsets_size)
{
	...
	if(best_fit == NULL) {
		printk(KERN_ERR "binder: %d: binder_alloc_buf size %d failed, no address space\n", proc->pid, size);
		return NULL;
	}
	if(n == NULL) {
	...
	if(binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_INFO "binder: %d: binder_alloc_buf size %d got buffer %p size %d\n", proc->pid, size, buffer, buffer_size);

	has_page_addr = (void *)(((size_t)buffer->data + buffer_size) & PAGE_MASK);
	if(n == NULL) {
...
static void binder_delete_free_buffer(struct binder_proc *proc, struct binder_buffer *buffer)
{
	...
	if(buffer_end_page(prev) == buffer_start_page(buffer)) {
		...
		if(buffer_end_page(prev) == buffer_end_page(buffer))
			free_page_end = 0;
		if(binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			printk(KERN_INFO "binder: %d: merge free, buffer %p share page with %p\n", proc->pid, buffer, prev);
	}
	...
	if(free_page_start || free_page_end) {
		if(binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			printk(KERN_INFO "binder: %d: merge free, buffer %p do not share page%s%s with with %p or %p\n", 
...
static void binder_free_buf(struct binder_proc *proc, struct binder_buffer *buffer)
{
	...
	if(binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_INFO "binder: %d: binder_free_buf %p size %d buffer_size %d\n", proc->pid, buffer, size, buffer_size);
...
static int
binder_inc_node(struct binder_node *node, int strong, int internal, struct list_head *target_list)
{
	if(strong) {
		if(internal) {
...
binder_get_ref_for_node(struct binder_proc *proc, struct binder_node *node)
{
	...
	if(node) {
		...
		if(binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
			printk(KERN_INFO "binder: %d new ref %d desc %d for dead node\n", proc->pid, new_ref->debug_id, new_ref->desc);
	}
	return new_ref;
}
...
{
	if(binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
		printk(KERN_INFO "binder: %d delete ref %d desc %d for node %d\n", ref->proc->pid, ref->debug_id, ref->desc, ref->node->debug_id);
	...
	if(ref->death) {
		if(binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
			printk(KERN_INFO "binder: %d delete ref %d desc %d has death notification\n", ref->proc->pid, ref->debug_id, ref->desc);
...
static int
binder_inc_ref(struct binder_ref *ref, int strong, struct list_head *target_list)
{
	int ret;
	if(strong) {
		if(ref->strong == 0) {
...
static void
binder_pop_transaction(struct binder_thread *target_thread, struct binder_transaction *t)
{
	if(target_thread) {
...
binder_send_failed_reply(struct binder_transaction *t, uint32_t error_code)
{
	...
	while(1) {
		...
		if(target_thread) {
			if(target_thread->return_error != brOK &&
			   target_thread->return_error2 == brOK) {
				target_thread->return_error2 = target_thread->return_error;
				target_thread->return_error = brOK;
			}
			if(target_thread->return_error == brOK) {
			...
			if(binder_debug_mask & BINDER_DEBUG_FAILED_TRANSACTION)
				printk(KERN_INFO "binder: send failed reply for transaction %d, target dead\n", t->debug_id);

			binder_pop_transaction(target_thread, t);
			if(next == NULL) {
				if(binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
					printk(KERN_INFO "binder: reply failed, no target thread at root\n");
				return;
			}
			t = next;
			if(binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
				printk(KERN_INFO "binder: reply failed, no target thread -- retry %d\n", t->debug_id);
		}
	}
}
...
static void
binder_transaction(struct binder_proc *proc, struct binder_thread *thread, binder_transaction_data_t *tr, int reply)
{
	...
	if(reply) {
		...
		if(in_reply_to == NULL) {
			if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
				printk(KERN_INFO "binder: %d:%d got reply transaction with no transaction stack\n",
					   proc->pid, thread->pid);
			return_error = brFAILED_REPLY;
			goto err_empty_call_stack;
		}
		if(in_reply_to->to_thread != thread) {
			if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
				printk(KERN_INFO "binder: %d:%d got reply transaction with bad transaction stack\n",
					   proc->pid, thread->pid);
			return_error = brFAILED_REPLY;
			in_reply_to = NULL;
		...
		if(target_thread == NULL) {
			return_error = brDEAD_REPLY;
			goto err_dead_binder;
		}
		if(target_thread->transaction_stack != in_reply_to) {
			if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
				printk(KERN_INFO "binder: %d:%d got reply transaction with bad target transaction stack\n",
					   proc->pid, thread->pid);
			return_error = brFAILED_REPLY;
			in_reply_to = NULL;
			target_thread = NULL;
		...
		if(tr->target.handle) {
			...
			if(ref == NULL) {
				if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
					printk(KERN_INFO "binder: %d:%d got transaction to invalid handle\n",
						   proc->pid, thread->pid);
				return_error = brFAILED_REPLY;
	...
	if(binder_debug_mask & BINDER_DEBUG_TRANSACTION) {
		if(reply)
			printk(KERN_INFO "binder: %d:%d bcREPLY %d -> %d:%d, data %p-%p size %d-%d\n",
			       proc->pid, thread->pid, t->debug_id, target_proc->pid, target_thread->pid,
			       tr->data.ptr.buffer, tr->data.ptr.offsets,
			       tr->data_size, tr->offsets_size);
		else
			printk(KERN_INFO "binder: %d:%d bcTRANSACTION %d -> %d - node %d, data %p-%p size %d-%d\n",
	...
	t->code = tr->code;
	t->flags = tr->flags | tfSynchronous;
	...
	if(copy_from_user(t->buffer->data, tr->data.ptr.buffer, tr->data_size)) {
		if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
			printk(KERN_INFO "binder: %d:%d got transaction with invalid data ptr\n",
				   proc->pid, thread->pid);
		return_error = brFAILED_REPLY;
	...
	if(copy_from_user(offp, tr->data.ptr.offsets, tr->offsets_size)) {
		if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
			printk(KERN_INFO "binder: %d:%d got transaction with invalid offsets ptr\n",
				   proc->pid, thread->pid);
		return_error = brFAILED_REPLY;
		goto err_copy_data_failed;
	}
	...
	for(; offp < off_end; offp++) {
		flat_binder_object_t *fp;
		if(*offp > t->buffer->data_size - sizeof(*fp)) {
			if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
				printk(KERN_INFO "binder: %d:%d got transaction with invalid offset, %d\n",
					   proc->pid, thread->pid, *offp);
			return_error = brFAILED_REPLY;
			goto err_bad_offset;
		}
		fp = (flat_binder_object_t *)(t->buffer->data + *offp);
		switch(fp->type) {
			...
			case kPackedLargeBinderWeakType: {
				...
				if(fp->type == kPackedLargeBinderType)
					fp->type = kPackedLargeBinderHandleType;
				else
					fp->type = kPackedLargeBinderWeakHandleType;
				fp->handle = ref->desc;
				binder_inc_ref(ref, fp->type == kPackedLargeBinderHandleType, &thread->todo);
				if(binder_debug_mask & BINDER_DEBUG_TRANSACTION)
			...
			case kPackedLargeBinderWeakHandleType: {
				...
				if(ref == NULL) {
					if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
						printk(KERN_INFO "binder: %d:%d got transaction with invalid handle, %ld\n",
							   proc->pid, thread->pid, fp->handle);
					return_error = brFAILED_REPLY;
					goto err_binder_get_ref_failed;
				}
				if(ref->node->proc == target_proc) {
					if(fp->type == kPackedLargeBinderHandleType)
						fp->type = kPackedLargeBinderType;
					else
						fp->type = kPackedLargeBinderWeakType;
					fp->binder = ref->node->ptr;
					fp->cookie = ref->node->cookie;
					binder_inc_node(ref->node, fp->type == kPackedLargeBinderType, 0, NULL);
					if(binder_debug_mask & BINDER_DEBUG_TRANSACTION)
					...
					binder_inc_ref(new_ref, fp->type == kPackedLargeBinderHandleType, NULL);
					if(binder_debug_mask & BINDER_DEBUG_TRANSACTION)
			...
			case kPackedLargeFileDescriptorType: {
				int target_fd;
				struct file * file;
				file = fget(fp->handle);
				if(file == NULL) {
					if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
						printk(KERN_INFO "binder: %d:%d got transaction with invalid fd, %ld\n",
							   proc->pid, thread->pid, fp->handle);
					return_error = brFAILED_REPLY;
			...
			default:
				if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
					printk(KERN_INFO "binder: %d:%d got transaction with invalid object type, %lx\n",
	...
	if(reply) {
		t->buffer->async_transaction = 0;
		binder_pop_transaction(target_thread, in_reply_to);
	}
	else if(t->flags & tfSynchronous){
		t->buffer->async_transaction = 0;
		t->need_reply = 1;
...
err_no_context_mgr_node:
	...
	BUG_ON(thread->return_error != brOK);
	if(in_reply_to) {
		thread->return_error = brTRANSACTION_COMPLETE;
...
binder_transaction_buffer_release(struct binder_proc *proc, struct binder_buffer *buffer, size_t *failed_at)
{
	...
	for(; offp < off_end; offp++) {
		...
		fp = (flat_binder_object_t *)(buffer->data + *offp);
		switch(fp->type) {
			case kPackedLargeBinderType:
			case kPackedLargeBinderWeakType: {
				struct binder_node *node = binder_get_node(proc, fp->binder);
				...
				binder_dec_node(node, fp->type == kPackedLargeBinderType, 0);
			} break;
			case kPackedLargeBinderHandleType:
			case kPackedLargeBinderWeakHandleType: {
				struct binder_ref *ref = binder_get_ref(proc, fp->handle);
				...
				binder_dec_ref(ref, fp->type == kPackedLargeBinderHandleType);
			} break;
...
binder_thread_write(struct binder_proc *proc, struct binder_thread *thread, void __user *buffer, int size, signed long *consumed)
{
	...
	while(ptr < end && thread->return_error == brOK) {
		if(get_user(cmd, (uint32_t __user *)ptr))
			return -EFAULT;
		ptr += sizeof(uint32_t);
		if(cmd < ARRAY_SIZE(binder_stats.bc)) {
			binder_stats.bc[cmd]++;
			proc->stats.bc[cmd]++;
			thread->stats.bc[cmd]++;
		}
		switch(cmd) {
			...
			case bcACQUIRE_DONE: {
				...
				binder_dec_node(node, cmd == bcACQUIRE_DONE, 0);
				if(binder_debug_mask & BINDER_DEBUG_USER_REFS)
					printk(KERN_INFO "binder: %d:%d %s node %d ls %d lw %d\n",
					       proc->pid, thread->pid, cmd == bcINCREFS_DONE ? "bcINCREFS_DONE" : "bcACQUIRE_DONE", node->debug_id, node->local_strong_refs, node->local_weak_refs);
				break;
			}
			case bcATTEMPT_ACQUIRE:
				printk(KERN_ERR "binder: bcATTEMPT_ACQUIRE not supported\n");
				return -EINVAL;
			case bcACQUIRE_RESULT:
				printk(KERN_ERR "binder: bcACQUIRE_RESULT not supported\n");
				return -EINVAL;
			...
			case bcFREE_BUFFER: {
				...
				if(buffer == NULL) {
					if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
						printk(KERN_INFO "binder: %d:%d bcFREE_BUFFER u%p no match\n", proc->pid, thread->pid, data_ptr);
					break;
				}
				if(!buffer->allow_user_free) {
					if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
						printk(KERN_INFO "binder: %d:%d bcFREE_BUFFER u%p matched unreturned buffer\n", proc->pid, thread->pid, data_ptr);
					break;
				}
				if(binder_debug_mask & BINDER_DEBUG_FREE_BUFFER)
					printk(KERN_INFO "binder: %d:%d bcFREE_BUFFER u%p found buffer %d for %s transaction\n",
			...
			case bcREPLY: {
				...
				if(copy_from_user(&tr, ptr, sizeof(tr)))
					return -EFAULT;
				ptr += sizeof(tr);
				binder_transaction(proc, thread, &tr, cmd == bcREPLY);
				break;
			}

			case bcREGISTER_LOOPER:
				if(binder_debug_mask & BINDER_DEBUG_THREADS)
					printk(KERN_INFO "binder: %d:%d bcREGISTER_LOOPER\n",
					       proc->pid, thread->pid);
				if(thread->looper & BINDER_LOOPER_STATE_ENTERED) {
					thread->looper |= BINDER_LOOPER_STATE_INVALID;
					if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
						printk(KERN_INFO "binder: %d:%d ERROR: bcREGISTER_LOOPER called after bcENTER_LOOPER\n",
						       proc->pid, thread->pid);
				}
				else if(proc->requested_threads == 0) {
					thread->looper |= BINDER_LOOPER_STATE_INVALID;
					if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
						printk(KERN_INFO "binder: %d:%d ERROR: bcREGISTER_LOOPER called without request\n",
						       proc->pid, thread->pid);
				}
				else {
			...
			case bcENTER_LOOPER:
				if(binder_debug_mask & BINDER_DEBUG_THREADS)
					printk(KERN_INFO "binder: %d:%d bcENTER_LOOPER\n",
					       proc->pid, thread->pid);
				if(thread->looper & BINDER_LOOPER_STATE_REGISTERED) {
					thread->looper |= BINDER_LOOPER_STATE_INVALID;
					if(binder_debug_mask & BINDER_DEBUG_USER_ERROR)
						printk(KERN_INFO "binder: %d:%d ERROR: bcENTER_LOOPER called after bcREGISTER_LOOPER\n",
						       proc->pid, thread->pid);
				}
			...
			case bcEXIT_LOOPER:
				if(binder_debug_mask & BINDER_DEBUG_THREADS)
					printk(KERN_INFO "binder: %d:%d bcEXIT_LOOPER\n",
					       proc->pid, thread->pid);
			...
			case bcCLEAR_DEATH_NOTIFICATION: {
				...
				if(binder_debug_mask & BINDER_DEBUG_DEATH_NOTIFICATION)
					printk(KERN_INFO "binder: %d:%d %s %p ref %d desc %d s %d w %d for node %d\n",
					       proc->pid, thread->pid,
					       cmd == bcREQUEST_DEATH_NOTIFICATION ?
					       "bcREQUEST_DEATH_NOTIFICATION" :
					       "bcCLEAR_DEATH_NOTIFICATION",
					       cookie, ref->debug_id, ref->desc,
					       ref->strong, ref->weak, ref->node->debug_id);

				if(cmd == bcREQUEST_DEATH_NOTIFICATION) {
					if(ref->death)
						return -EINVAL;
			...
			case bcDEAD_BINDER_DONE: {
				...
				if(binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
					printk(KERN_INFO "binder: %d:%d bcDEAD_BINDER_DONE %p found %p\n",
...
{
	if(cmd < ARRAY_SIZE(binder_stats.br)) {
		binder_stats.br[cmd]++;
		proc->stats.br[cmd]++;
		thread->stats.br[cmd]++;
	}
...
int 
binder_thread_read(struct binder_proc *proc, struct binder_thread *thread, void  __user *buffer, int size, signed long *consumed)
{
	void __user *ptr = buffer + *consumed;
	void __user *end = buffer + size;
	...
	if(*consumed == 0) {
		if(put_user(brNOOP, (uint32_t __user *)ptr))
			return -EFAULT;
		ptr += sizeof(uint32_t);
	}
...
retry:
	...
	if(thread->return_error != brOK && ptr < end) {
		if(thread->return_error2 != brOK) {
			if(put_user(thread->return_error2, (uint32_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(uint32_t);
			if(ptr == end)
				goto done;
			thread->return_error2 = brOK;
		}
		if(put_user(thread->return_error, (uint32_t __user *)ptr))
			return -EFAULT;
		ptr += sizeof(uint32_t);
		thread->return_error = brOK;
		goto done;
	}
	...
	if(wait_for_proc_work)
		ret = wait_event_interruptible(proc->wait, !list_empty(&proc->todo) || (thread->looper & BINDER_LOOPER_STATE_NEED_RETURN));
	else
		ret = wait_event_interruptible(thread->wait, !list_empty(&thread->todo) || thread->return_error != brOK || (thread->looper & BINDER_LOOPER_STATE_NEED_RETURN));
	...
	while(1) {
		...
		else if(!list_empty(&proc->todo) && thread->transaction_stack == NULL)
		...
		switch(w->type) {
			...
			case BINDER_WORK_TRANSACTION_COMPLETE: {
				...
				if(binder_debug_mask & BINDER_DEBUG_TRANSACTION_COMPLETE)
					printk(KERN_INFO "binder: %d:%d brTRANSACTION_COMPLETE\n",
					       proc->pid, thread->pid);
			...
			case BINDER_WORK_CLEAR_DEATH_NOTIFICATION: {
				...
				if(w->type == BINDER_WORK_CLEAR_DEATH_NOTIFICATION)
					cmd = brCLEAR_DEATH_NOTIFICATION_DONE;
				else
					cmd = brDEAD_BINDER;
				if(put_user(cmd, (uint32_t __user *)ptr))
					return -EFAULT;
		...
		tr.code = t->code;
		tr.flags = t->flags;
		set_user_nice(current, t->priority);
		tr.priority = 0;
		...
		if(binder_debug_mask & BINDER_DEBUG_TRANSACTION)
			printk(KERN_INFO "binder: %d:%d %s %d %d:%d, cmd %d size %d-%d ptr %p-%p\n",
			       proc->pid, thread->pid,
			       (cmd == brTRANSACTION) ? "brTRANSACTION" : "brREPLY",
		...
		if(cmd == brTRANSACTION && (t->flags & tfSynchronous)) {
	...
	*consumed = ptr - buffer;
	if(proc->requested_threads + proc->ready_threads == 0 &&
	   ...
	   (thread->looper & (BINDER_LOOPER_STATE_REGISTERED | BINDER_LOOPER_STATE_ENTERED)) /* the user-space code fails to spawn a new thread if we leave this out */ ) {
		proc->requested_threads++;
		if(binder_debug_mask & BINDER_DEBUG_THREADS)
			printk(KERN_INFO "binder: %d:%d brSPAWN_LOOPER\n",
			       proc->pid, thread->pid);
		if(put_user(brSPAWN_LOOPER, (uint32_t __user *)buffer))
			return -EFAULT;
	}
	return 0;
}
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case BINDER_VERSION:
			...
			if(put_user(BINDER_CURRENT_PROTOCOL_VERSION, &((binder_version_t*)ubuf)->protocol_version)) {
				ret = -EINVAL;
				goto err;
			}
			break;
	...
	while(!list_empty(list)) {
		...
		switch(w->type) {
			case BINDER_WORK_TRANSACTION: {
				...
				if(t->buffer->target_node)
					binder_send_failed_reply(t, brDEAD_REPLY);
			} break;
			case BINDER_WORK_TRANSACTION_COMPLETE: {
				kfree(w);
				binder_stats.obj_deleted[BINDER_STAT_TRANSACTION_COMPLETE]++;
...
static int binder_release(struct inode *nodp, struct file *filp)
{
	...
	while((n = rb_first(&proc->threads))) {
		...
		if(send_reply)
			binder_send_failed_reply(send_reply, brDEAD_REPLY);
...
{
	buf += snprintf(buf, end - buf, "%s %d: %p from %d:%d to %d:%d code %x flags %x pri %d r%d",
...
static char *print_binder_thread(char *buf, char *end, struct binder_thread *thread, int print_always)
{
	...
	list_for_each_entry(w, &thread->todo, entry) {
		...
		switch(w->type) {
			case BINDER_WORK_TRANSACTION:
				buf = print_binder_transaction(buf, end, "    pending transaction", container_of(w, struct binder_transaction, work));
				break;
			case BINDER_WORK_TRANSACTION_COMPLETE:
				buf += snprintf(buf, end - buf, "    transaction complete\n");
				break;
			case BINDER_WORK_NODE: {
				struct binder_node *node = container_of(w, struct binder_node, work);
				buf += snprintf(buf, end - buf, "    node work %d: u%p c%p\n", node->debug_id, node->ptr, node->cookie);
				break;
			}
			default:
				buf += snprintf(buf, end - buf, "  unknown work: type %d\n", w->type);
				break;
		}
	}
...
static char *print_binder_proc(char *buf, char *end, struct binder_proc *proc, int print_all)
{
	...
	if(print_all) {
		for(n = rb_first(&proc->nodes); n != NULL && buf < end; n = rb_next(n))
			buf = print_binder_node(buf, end, rb_entry(n, struct binder_node, rb_node));
	...
	list_for_each_entry(w, &proc->todo, entry) {
		...
		switch(w->type) {
			...
			case BINDER_WORK_NODE: {
				struct binder_node *node = container_of(w, struct binder_node, work);
				buf += snprintf(buf, end - buf, "  node work u%p: c%p k%p\n", node->ptr, node->cookie, node);
				break;
			}
			case BINDER_WORK_DEAD_BINDER:
				buf += snprintf(buf, end - buf, "  has dead binder\n");
				break;
			case BINDER_WORK_DEAD_BINDER_AND_CLEAR:
				buf += snprintf(buf, end - buf, "  has cleared dead binder\n");
				break;
			case BINDER_WORK_CLEAR_DEATH_NOTIFICATION:
				buf += snprintf(buf, end - buf, "  has cleared deth notification\n");
				break;
			default:
				buf += snprintf(buf, end - buf, "  unknown work: type %d\n", w->type);
				break;
...
static char *print_binder_transaction_log_entry(char *buf, char *end, struct binder_transaction_log_entry *e)
{
	buf += snprintf(buf, end - buf, "%d: %s from %d:%d to %d:%d node %d handle %d size %d:%d\n",
	                e->debug_id, e->call ? "call" : "return", e->from_proc,
	                e->from_thread, e->to_proc, e->to_thread, e->to_node,
	                e->target_handle, e->data_size, e->offsets_size);
	return buf;
}

static int binder_read_proc_transaction_log(char *page, char **start, off_t off,
                                            int count, int *eof, void *data)
{
